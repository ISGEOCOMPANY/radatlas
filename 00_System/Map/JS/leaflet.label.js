...
(function () {
    L.labelVersion = "0.2.2-dev", L.Label = L.Class.extend({
        includes: L.Mixin.Events, options: {
            className: "", clickable: !1, direction: "right", noHide: !1, offset: [0, 0], opacity: 1, zoomAnimation: !0
        },
        ...
        close: function () {
            var t = this._map; t && (L.Browser.touch && !this.options.noHide && L.DomEvent.off(this._container, "click", this.close), t.removeLayer(this))
        },
        ...
        _updateContent: function () {
            this._content && this._map && this._prevContent !== this._content && "string" == typeof this._content && (this._container.innerHTML = this._content, this._prevContent = this._content,
        ...
        _setPosition: function (t) {
            var e = this._map,
                ...
                lMapZoom = this._map._animateToZoom ? this._map._animateToZoom : this._map._zoom,
                lObjMinZoom = this.options.minZoom ? this.options.minZoom : this._map._layersMinZoom,
                lObjMaxZoom = this.options.maxZoom ? this.options.maxZoom : this._map._layersMaxZoom;

            if (lObjMinZoom <= lMapZoom && lObjMaxZoom >= lMapZoom) {
                L.DomUtil.removeClass(i, "leaflet-label-hide");
            }
            else {
                L.DomUtil.addClass(i, "leaflet-label-hide");
                return;
            }

            if ("right" === s) {
                (
                t = t.add(L.point(-l.x + 5, l.y - parseInt(b / 2))))
            }
            else if ("left" === s) {
                (
                t = t.add(L.point(-l.x - a - 5, l.y - parseInt(b / 2))))
            }
            else if ("top" === s) {
                (
                t = t.add(L.point(-l.x - parseInt(a / 2), l.y - b - parseInt(b / 16))))
            }
            else if ("bottom" === s) {
                (
                t = t.add(L.point(-l.x - parseInt(a / 2), l.y + parseInt(b / 16))))
            }
            else if ("center" === s) {
                (
                t = t.add(L.point(-l.x - parseInt(a / 2), l.y - parseInt(b / 2))))
            }
    ...
    L.Path.include({
        ...
        _moveLabel: function (t) {
            this.label.setLatLng(t.latlng)
        }, _hideLabel: function () { this.label.close() }
    }),
    L.Map.include({
        showLabel: function (t) { return this.addLayer(t) }
        ...
        updateLabelContent: function (t) { this.invoke("updateLabelContent", t) }
    })
})(this, document);